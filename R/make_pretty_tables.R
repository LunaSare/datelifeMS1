...
make_pretty_table <- function(some_table, duplicated_criteria) {
  # assign variables:
  ...
  if (missing(duplicated_criteria)) {
    ...
    duplicated_criteria <- duplicated_criteria
  }
  ...
  if (sum(duplicated(taxon_pairs_ref)) == 0) {
    message("There are no duplicated data.")
  ...
  row_order <- order(pretty_calibs$mrca_node_number, pretty_calibs$MaxAge, pretty_calibs$reference, pretty_calibs$taxonA, pretty_calibs$taxonB)
  ...
  pretty_calibs <- some_table[ , cols2keep_order]
