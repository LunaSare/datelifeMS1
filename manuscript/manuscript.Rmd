...
To date a tree topology when branch lengths are unavailable, DateLife implements the Branch Length Adjuster (BLADJ) algorithm [@Webb2008; @webb2005phylomatic], which only requires a tree topology with no branch lengths and at least two node ages to use as calibrations, one for the tree root and one for any internal node of the topology.
...
We found that node ages from the original studies, and ages estimated using all other age data available are largely correlated (Fig. 4).
