...
The DateLife algorithm is fully implemented using the R language. The latest stable version of the R package `datelife` -- v`r packageVersion("datelife")`, is available from the CRAN repository [@datelifeR], and relies on functionalities from various biological R packages:
ape [@paradis2004],
...
Summarizing options implemented include Super Distance Matrix method [SDM, @Criscuolo2006] and summary statistics.
...
<!-- We need to think about the root! is it comparable across source chronograms?? not all of them have the same sampling... so probably not the same root and thus not comparable. Address this...   ANSWERED: -->
