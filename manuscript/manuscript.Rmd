...
At the time of writing of this manuscript, `datelife`'s database had `r length(opentree_chronograms$trees)` chronograms. The database is cuurently constructed entirely from OpenTree's tree repository, the only public tree repository from where one can currently get chronograms to construct a database. This represents `r round(length(opentree_chronograms$trees)*100/3987, digits = 2)`% of the largest existing chronogram database, TimeTree, which has a collection of 3,998 chronograms as of `r format(Sys.Date(), format="%B %d %Y")`. Unfortunately, TimeTree's database is not open for scientific reuse nor automatised data mining [@Kumar2017].
