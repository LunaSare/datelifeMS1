...
6. A single summary chronogram resulting from summarizing age data generated using the options described below.
...
To estimate branch lengths proportional to substitution rates on these topologies, DateLife can mine the Barcode of Life Data System, BOLD [@ratnasingham2007bold] to obtain genetic markers for the input taxa.
These markers are aligned with MUSCLE [@edgar2004muscle; or MAFFT, @katoh2009multiple]. 
This alignment can be used to estimate branch lengths on input topologies that lack branch lengths.
Currently, branch length reconstruction is performed with parsimony and the likelihood of the phylogenetic tree given a sequence alignment is computed [@schliep2011phangorn].
...
\NOTE:do you know how it decides which one to drop if two calibrations conflict? The parent or the child? 
...
[@Smith2012].
The MrBayes  [@Huelsenbeck2001; @Ronquist2003] apprach in datelife uses the secondary calibrartions as priors on node ages. \NOTE: is this true??
