...
DateLife can also reconstruct branch lengths proportional to substitution rates on the fixed tree topology using available genetic data from BOLD.
...
_**Summarizing node ages.--**_ DateLife summarizes matched calibrations  ??? How is 'calibrations' defined here? the calibrations from congruification, or the pairwise distances??? into a single patristic distance matrix using different methods. 
...
We illustrate the `datelife` algorithm using the family of true finches, Fringillidae as an example.
