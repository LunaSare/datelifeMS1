...
Here we present the DateLife project and its core software application, available as an R package [@datelifeR], and as an online Rshiny interactive website at www.datelife.org/query/, which captures data from published chronograms, and make these data readily accessible to users for reuse and reanalysis.
...
<!-- We are using OpenTree's TNRS, Boyle described TNRS first, right?  -->
...
Published chronograms can be added to Phylesystem by any user, at any time, and are immediately publicly available (https://tree.opentreeoflife.org/curator). This facilitates an immediate update of DateLife's chronogram database to include new chronogram data on a following search.
...
6. Last but not least, single summary chronograms resulting from summarizing age data from available source chronograms, as follows:

_**Choosing a topology.--**_ DateLife requires a tree topology to summarize age data upon. We recommend that users provide a tree topology as input from the literature, or one of their own making. If no topology is provided, DateLife automatically subsets one from the OpenTree synthetic tree [@opentreeoflife2019synth]. Alternatively, DateLife can reconstruct a tree with branch lengths proportional to substitution rates from a starting tree topology using genetic data from the Barcode of Life Data System, BOLD [@ratnasingham2007bold], or combine topologies from source chronograms using a supertree approach.
...
_**Summarizing congruified ages.--**_ 
<!-- we should calculate summary statistics per node -->
...
For each congruent node, the pairwise distance that traverse that node are summarized into a single summary matrix using classic statistics (mean, median, minimum and maximum ages). These single summary ages per taxon pair are then used as secondary calibrations to date the tree topology.  
<!--
Under the SDM criterion ??? what happens??
Summarizing options implemented include Super Distance Matrix method [SDM, @Criscuolo2006] 
-->


_**Dating the tree topology.--**_ 
By default, DateLife implements the Branch Length Adjuster (BLADJ) algorithm to obtain a fully dated topology. BLADJ fixes node ages that have calibration data, and distributes time between nodes with no data evenly between nodes with calibration data. This minimizes age variance in the resulting chronogram [@Webb2008].
...
Finally, users can save all source and summary chronograms in formats that permit reuse and reanalyses (newick and R "phylo" format), as well as visualize and compare results graphically, or construct their own graphs using \texttt{datelife}'s chronogram plot generation functions.
