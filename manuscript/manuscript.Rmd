...
\item To construct summary trees from patristic distance matrices, one can use a clustering method like Neighbor Joining. We implemented this in the package. However, it is know that these methods can return toologies that are not biologically plausible (Example and citation). Hence, we implement
a different workflow, in which we use a fixed topology taken from the literature or from expert
phylogenetic information (such as the OpenTree synthetic tree), and we use the patristic distances as calibrations to date that topology (with BLADJ for now). TODO ADD FIGURE: mock example explaining differenc ebetween using summary ages and raw ages to calibrate a topology. Then show it in the biological example. This will show what happens if a tree (phylogenetic conflict) or node (age) do not agree.
