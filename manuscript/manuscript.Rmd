...
In addition, nodes that are sandwiched between a calibrated node and a calibrated root have less freedom of movement and hence narrower confidence intervals [@vos2004reconstructing], this inflates precision for nodes without calibrations but does not necessarily improve accuracy of the estimated ages.
...
While most chronograms in DateLife's database are constructed using primary calibrations (molecular substitution rates or ages obtained from the fossil record or geological events), DateLife also summarizes chronograms using secondary calibrations (ages coming from other chronograms).
The scientific community has generally more confidence in chronograms generated from a single analysis, where fossil data constitute the primary source of calibrations [@sauquet2013practical; @schenk2016sec; @antonelli2017supersmart].
@graur2004reading cautioned on the increased error and uncertainty in estimated node ages when using secondary calibrations in dating analyses, and more recently, @schenk2016sec found that divergence times obtained using secondary calibrations with Bayesian inference methods are significantly younger than those obtained using primary calibrations. While this trend still needs to be evaluated with alternative methods of divergence time estimation,
 many efforts and discussions on best practices for implementing primary calibrations have been advanced in recent years,
for example, using fossils that have been widely discussed and curated as calibrations to date other trees and making sure that all data reflect a coherent evolutionary history [@sauquet2013practical], as for example done by @antonelli2017supersmart. <!-- Will this be implemented in future `datelife` versions? Probably not. -->

Obtaining vetted primary calibrations from the litterature is another bottleneck for facilitating obtention of chronograms.
The Fossil Calibration Database provides availability of 220 primary calibration points encompassing flowering plants and metazoans, that have been curated by experts and used in peer-reviewd publications [@ksepka2015fossil]. This database facilitates the use of expert primary fossil calibrations in new phylogenetic dating analysis. <!--though these still have room to grow... BCO: Polite way to say 'no new additions in four years?' LLSR: lol, yes-->
