...
The BLADJ algorithm requires a root age estimate to run. If there is no information on the age of the root in the chronogram database, users can provide an estimate from the literature. If none is provided, DateLife will choose an age for the root so that it can return a dated topology. It will also provide a conspicuous warning message so that users are aware that the root of the chronogram does not have information available in the chronogram database, along with suggestions on how the user can provide an appropriate age for the root.
...
Median summary age data for node "n2" was excluded as final calibration because it is older than the median age of a more inclusive node, "n1" (Fig. 3c4).
