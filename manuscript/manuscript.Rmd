...
Up to the time this manuscript was written, `datelife`'s chronogram database had `r length(opentree_chronograms$trees)` chronograms, pulled entirely from OpenTree's tree repository, the only public tree repository from where `datelife` can currently get chronograms to construct its database. This represents `r round(length(opentree_chronograms$trees)*100/3987, digits = 2)`% of the largest existing chronogram database, TimeTree, which has a collection of 3,998 chronograms as of `r format(Sys.Date(), format="%B %d %Y")`. Unfortunately, TimeTree's database is not open for scientific reuse nor automated data mining [@Kumar2017].
