...
%DIF DEL manuscript/manuscript-1st-submission.tex   Mon Jan 23 15:36:09 2023
%DIF ADD manuscript/manuscript.tex                  Mon Jan 23 15:37:12 2023
...
DateLife's core software application consists of the R package \texttt{datelife}. Its current stable version -- v0.6.6, is available from the \DIFdelbegin \DIFdel{The }\DIFdelend Comprehensive R Archive Network (CRAN) repository (Sanchez-Reyes et al., 2022), and relies on functionalities from various biological R packages:
...
DateLife starts by processing an input consisting of at least \DIFdelbegin \DIFdel{two taxon names, which }\DIFdelend \DIFaddbegin \DIFadd{one taxon scientific names. Two or more scientific names }\DIFaddend can be provided as a comma separated character string or as tip labels on a tree.
...
The processed }\DIFaddend taxon names are saved as an R object of a newly defined class, \texttt{datelifeQuery}, that is used in the following steps. This object contains the \DIFdelbegin \DIFdel{standardized names }\DIFdelend \DIFaddbegin \DIFadd{names standardized to the taxonomy of choice}\DIFaddend , the corresponding OTT \DIFdelbegin \DIFdel{identification }\DIFdelend \DIFaddbegin \DIFadd{id }\DIFaddend numbers, and the topology of the input tree if \DIFdelbegin \DIFdel{any }\DIFdelend \DIFaddbegin \DIFadd{one }\DIFaddend was provided.
...
To \DIFdelbegin \DIFdel{combine topologies from source chronograms into a single summary (or supertree) topology, the DateLife workflow }\DIFdelend \DIFaddbegin \DIFadd{do this DateLife first }\DIFaddend identifies the source chronograms that form a grove, roughly, a sufficiently overlapping set of taxa between trees, by implementing definition 2.8 for n-overlap from Ané et al. (2009).
...
For each congruent node, the pairwise distances that traverse that node are summarized into a single summary matrix using classic summary statistics (i.e., mean, median, minimum and maximum ages), and the Supermatrix Distance Method (SDM; Criscuolo, Berry, Douzery, \& Gascuel, 2006), which deforms patristic distance matrices by minimizing variance and then averaging them.
...
\DIFaddend By default, DateLife implements the Branch Length Adjuster (BLADJ\DIFaddbegin \DIFadd{; Webb et al., 2008; Webb \& Donoghue, 2005}\DIFaddend ) algorithm to obtain a fully dated topology. BLADJ \DIFaddbegin \DIFadd{is the only dating algorithm that can work with initial topologies without any branch length data.
}

\DIFadd{Alternatively, the user can chose phylogenetic dating options supported in DateLife that incorporate branch length information from the input topology in combination with the secondary calibrations:
...
\DIFadd{The latter methods can assign dates using a birth-death model (all of them????), they require initial branch lengths and thus require more time and expertise to run, which makes BLADJ the fastest and most practical method to obtain a dated tree.
}

\DIFadd{BLADJ }\DIFaddend fixes node ages that have calibration data, and distributes time between nodes with no data evenly between \DIFdelbegin \DIFdel{nodes with calibration data.
This }\DIFdelend \DIFaddbegin \DIFadd{calibrated nodes.
This has proven effective }\DIFaddend minimizes age variance in the resulting chronogram \DIFaddbegin \DIFadd{and useful for ecoogical analyses }\DIFaddend (Webb et al., 2008).
...
}\DIFdelend When there is conflict in ages between nodes with calibration data, BLADJ ignores node ages that are older than the age of a parent node.
BLADJ requires a root age estimate. If there is no information on the age of the root in the chronogram database, users can provide an estimate from the literature. If none is provided, DateLife \DIFdelbegin \DIFdel{assigns an arbitrary age to the root as 10\% older than the oldest age available within the group}\DIFdelend \DIFaddbegin \DIFadd{will not return a dated topology but provide a warning message along with suggestions on how the user can provide an age for the root so DateLife can run}\DIFaddend .

\DIFdelbegin \DIFdel{Alternative phylogenetic dating options supported in DateLife (MrBayes, PATHD8, TreePL) incorporate branch length information from the input topology in combination with the calibrations.
PATHd8 is a non-clock, rate-smoothing method (Britton et al.}\DIFdelend \DIFaddbegin \emph{\textbf{\DIFadd{Dating a tree with branch lengths--.}}}
...
To estimate branch lengths proportional to substitution rates for these topologies, DateLife currently implements a simple algorithm. First, it mines the Barcode of Life Data System, BOLD (Ratnasingham \& Hebert}\DIFaddend , 2007) to \DIFdelbegin \DIFdel{date trees.
treePL (Smith \& O'Meara, 2012) , is a semi-parametric, rate-smoothing, penalized likelihood dating method (Sanderson, 2002).
The MrBayes (Huelsenbeck \& Ronquist, 2001; Ronquist \& Huelsenbeck, 2003)approach in DateLife uses the calibrations as priors on node ages. }\DIFdelend \DIFaddbegin \DIFadd{obtain genetic markers for the input taxa.
Mined genetic sequences are aligned with MUSCLE (Edgar, 2004) (by default) or MAFFT (Katoh, Asimenos, \& Toh, 2009).
}

\DIFadd{The BOLD sequence alignment is then used to reconstruct branch lengths with the accelerated transformation (ACCTRAN) parsimony algorithm, which resolves ambiguous character optimization, by assigning changes along branches of the tree as close to the root as possible (Agnarsson \& Miller, 2008). This algorithm work rally fast and allows getting initial branch lengths that ar ethen optimized using ML.
...
\DIFaddbegin \DIFadd{Relative branch length information provides key data for phylogenetic dating, especially for nodes without secondary calibrations available. Yet, topologies without branch lengths can also be dated.
}
...
By default, \DIFaddbegin \DIFadd{to do this, }\DIFaddend DateLife uses the topology from OpenTree's synthetic tree that contains \DIFdelbegin \DIFdel{the species in the search queryto summarize age data upon}\DIFdelend \DIFaddbegin \DIFadd{all taxa from the search query}\DIFaddend .
...
\DIFdelbegin \DIFdel{For downstream analyses, using alternative chronogram may deeply affect our inferences (Title \& Rabosky, 2016), particularly when studying }\DIFdelend \DIFaddbegin \DIFadd{The study of }\DIFaddend phenomena dependent on the timing of species diversification events, such as macroevolutionary processes\DIFaddbegin \DIFadd{, is affected by the usage of alternative chronograms that vary in topology (Rabosky, 2015; Title \& Rabosky, 2016)}\DIFaddend .

\DIFdelbegin \DIFdel{In ecology and conservation biology}\DIFdelend \DIFaddbegin \DIFadd{On the other hand}\DIFaddend , incorporating at least some data on lineage divergence times represents a relevant improvement for testing alternative hypothesis using phylogenetic distance \DIFaddbegin \DIFadd{in ecological and conservation biology studies }\DIFaddend (Webb et al., 2008).
Hence, DateLife's workflow features different ways of \DIFdelbegin \DIFdel{estimating }\DIFdelend \DIFaddbegin \DIFadd{generating }\DIFaddend node ages in the absence of \DIFdelbegin \DIFdel{calibrations }\DIFdelend \DIFaddbegin \DIFadd{calibration }\DIFaddend and branch length information for certain taxa.
\DIFdelbegin %DIFDELCMD < \enquote{Making up} %%%
\DIFdel{branch lengths }\DIFdelend \DIFaddbegin 

\DIFadd{Adding branch lengths sampled from a birth-death model in the absence of genetic data
has been found to improve insight in phylogeny-based analyses.
}

\DIFaddend is a common practice in scientific publications: Jetz et al. (2012), created a chronogram of all 9, 993 bird species, where 67\% had molecular data and the rest was simulated; Rabosky et al. (2018) created a chronogram of 31, 536 ray-finned fishes, of which only 37\% had molecular data; Smith and Brown (2018) constructed a chronogram of 353, 185 seed plants where only 23\% had molecular data.

\DIFaddbegin \DIFadd{Simulating branch lengths following a birth-death species diversification model for missing taxa in chronograms with non random sampling, lowers type I error (false positive, incorrectly accepting the null hypothesis of a constant-rate or temporally varying rate birth-death model) when analyzing changes in diversification rate using the gamma statistic (Cusimano, Stadler, \& Renner, 2012).
}

\DIFadd{Thomas et al. (2013) hypothesize that results of diversification analyses will be biased towards the birth-death model used to simulate branch lengths;
and note that }\enquote{the effects of missing species placement or polytomy resolution are less clear for other phylogeny-based analyses (e.g.~correlates of diversification, modelling trait evolution, community phylogenetics), and future work should test how the treatment of missing species influences both parameter estimation and type I and II errors}

\DIFaddend Notably, risks come with this practice.
\DIFaddbegin 

\DIFaddend Taken to the extreme, one could \DIFdelbegin \DIFdel{make }\DIFdelend \DIFaddbegin \DIFadd{generate }\DIFaddend a fully resolved, calibrated tree of all modern and extinct taxa using a single taxonomy and a single calibration, using polytomy resolution and branch \DIFdelbegin \DIFdel{estimation }\DIFdelend \DIFaddbegin \DIFadd{length simulation }\DIFaddend methods.
...
\leavevmode\DIFaddbegin \hypertarget{ref-cusimano2012new}{}%DIF > 
...
\leavevmode\DIFaddbegin \hypertarget{ref-freckleton2002phylogenetic}{}%DIF > 
...
\leavevmode\DIFaddbegin \hypertarget{ref-thomas2013pastis}{}%DIF > 
\DIFadd{Thomas, G. H., Hartmann, K., Jetz, W., Joy, J. B., Mimoto, A., \& Mooers, A. O. (2013). PASTIS: An r package to facilitate phylogenetic assembly with soft taxonomic inferences. }\emph{\DIFadd{Methods in Ecology and Evolution}}\DIFadd{, }\emph{4}(11)\DIFadd{, 1011--1017.
}
