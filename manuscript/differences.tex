...
%DIF DEL manuscript-2nd-submission.tex   Tue Nov  7 19:22:46 2023
%DIF ADD manuscript.tex                  Tue Nov 21 13:29:09 2023
...
Finally, node ages from matching chronograms are mapped \DIFaddbegin \DIFadd{using the congruification algorithm }\DIFaddend to corresponding nodes \DIFdelbegin \DIFdel{from a chosen tree topologyusing the congruification algorithm}\DIFdelend \DIFaddbegin \DIFadd{on a tree topology, either extracted from Open Tree of Life's synthetic phylogeny or one provided by the user}\DIFaddend . Congruified node ages are used as secondary calibrations to date the chosen topology, with or without initial branch lengths, using different phylogenetic dating methods such as BLADJ, treePL, PATHd8 and MrBayes.
...
\DIFdelbegin \DIFdel{Results }\DIFdelend \DIFaddbegin \DIFadd{Because the results }\DIFaddend from any software for scientific analysis can only be as good as the data used as input, we highlight the importance of considering the results of a DateLife analysis in the context of the input chronograms.
\DIFdelbegin \DIFdel{We encourage the use of DateLife to help }\DIFdelend \DIFaddbegin \DIFadd{DateLife can help to }\DIFaddend increase awareness of the existing disparities among alternative hypotheses of dates for the same diversification events, and to support exploration of the effect of alternative chronogram hypotheses on downstream analyses, providing a framework for a more informed interpretation of evolutionary results.
...
Word count: \DIFdelbegin \DIFdel{6707
}\DIFdelend \DIFaddbegin \DIFadd{7484
}\DIFaddend 
...
\DIFdelbegin \DIFdel{Figure 1 provides a graphical summary of the }\DIFdelend \DIFaddbegin \DIFadd{There are }\DIFaddend three main steps \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend the DateLife workflow: \DIFaddbegin \DIFadd{1) }\DIFaddend creating a search query, \DIFaddbegin \DIFadd{2) }\DIFaddend searching a database, and \DIFaddbegin \DIFadd{3) }\DIFaddend summarizing results from the search.
...
(\DIFdelbegin \DIFdel{Jun 11}\DIFdelend \DIFaddbegin \DIFadd{Nov 21}\DIFaddend , 2023),
DateLife's chronogram database latest version consist of 253 chronograms published in 187 different studies\DIFaddbegin \DIFadd{, encompassing close to 100 k (99 474) different species}\DIFaddend . It is curated from OpenTree's phylogenetic database, the Phylesystem, \DIFdelbegin \DIFdel{which constitutes an open source }\DIFdelend \DIFaddbegin \DIFadd{an open database }\DIFaddend of expert and peer-reviewed phylogenetic knowledge with rich metadata \DIFaddbegin \DIFadd{and a wide taxonomic scope }\DIFaddend (McTavish et al., 2015)\DIFdelbegin \DIFdel{, which allows automatic and reproducible assembly of our chronogram database. Datelife's chronogram database is navigable as an R data object within the }\texttt{\DIFdel{datelife}} %DIFAUXCMD
\DIFdel{R package}\DIFdelend \DIFaddbegin \DIFadd{. We expect DateLife's database to largely overlap with OpenTree's phylogenetic database taxonomic coverage, where Chordata and Embryophyta are nearly fully sampled. In contrast, Bacteria, Fungi, Nematoda, and Insecta, currently present a large gap between the number of named species and what has phylogenetic information in OpenTree's synthetic tree. It is likely that users working with the former groups will get results from a DateLife analysis. If none of the user's species are found, the software will indicate the lack of age data for the queried taxa in the database}\DIFaddend .

A unique feature of the Phylesystem is that any user can add new published, state-of-the-art chronograms any time, through OpenTree's curator application (\url{https://tree.opentreeoflife.org/curator}). \DIFaddbegin \DIFadd{Relying on an open source database permits an automatic and reproducible assembly of DateLife's chronogram database, which is stored and navigable as an R data object within the }\texttt{\DIFadd{datelife}} \DIFadd{R package. }\DIFaddend As chronograms are added to Phylesystem, they can be incorporated into the chronogram database \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{within }\DIFaddend the \texttt{datelife} R package, \DIFdelbegin \DIFdel{which is currently manually updated as new chronogram data is added to Phylesystem}\DIFdelend \DIFaddbegin \DIFadd{by manually triggering an update}\DIFaddend . The updated \DIFaddbegin \texttt{\DIFadd{datelife}} \DIFaddend database is assigned a new version number, followed by a package release on CRAN.
\DIFaddbegin \DIFadd{We encourage users to submit published chronograms to OpenTree's phylogenetic database, so that their taxon of interest can be included in future DateLife searches.
}\DIFaddend Users can directly run \texttt{datelife} functions to trigger an update of their local chronogram database, to incorporate any new chronograms to their DateLife analysis before \DIFdelbegin \DIFdel{an official }\DIFdelend \DIFaddbegin \DIFadd{a }\texttt{\DIFadd{datelife}} \DIFaddend database update is released on CRAN.
...
Total distance in units of million years (Myr) between taxon pairs within each source chronogram are stored as a patristic distance matrix\DIFdelbegin \DIFdel{(Fig.
1).
}\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend The matrix format speeds up extraction of pairwise taxon ages of any queried taxa, as opposed to searching the ancestor node of a pair of taxa in a \enquote{phylo} object or newick string.
...
Summary information is extracted from the \texttt{datelifeResult} object to inform decisions for subsequent steps in the analysis workflow. Basic summary information available to the user \DIFdelbegin \DIFdel{is}\DIFdelend \DIFaddbegin \DIFadd{includes}\DIFaddend :
...
The result is a single summary (\DIFdelbegin \DIFdel{or }\DIFdelend \DIFaddbegin \DIFadd{i.e., }\DIFaddend supertree) topology, that combines topologies from source chronograms in a grove.
...
\DIFdelbegin \DIFdel{When producing or obtaining a tree with branch lengths for a group of interest is not possible, DateLife can date a topology without branch lengths , obtained from OpenTree or by implementing the supertree approach described above, by applying the }\DIFdelend \DIFaddbegin \DIFadd{To date a tree topology when initial branch lengths are unavailable, DateLife implements the }\DIFaddend Branch Length Adjuster (BLADJ\DIFdelbegin \DIFdel{; }\DIFdelend \DIFaddbegin \DIFadd{) algorithm (}\DIFaddend Webb et al., 2008; Webb \& Donoghue, 2005)\DIFdelbegin \DIFdel{algorithm, which requires no initial branch lengths .
The algorithm starts by fixing }\DIFdelend \DIFaddbegin \DIFadd{, which only requires a tree topology with no branch lengths and at least two node ages to use as calibrations, one for the tree root and one for any internal node of the topology.
The BLADJ algorithm fixes }\DIFaddend ages for nodes with calibration data upon the given \DIFaddbegin \DIFadd{tree }\DIFaddend topology. Then, it \DIFdelbegin \DIFdel{distributes time for }\DIFdelend \DIFaddbegin \DIFadd{assigns ages to }\DIFaddend nodes with no \DIFdelbegin \DIFdel{data }\DIFdelend \DIFaddbegin \DIFadd{available age information by distributing time }\DIFaddend evenly between calibrated nodes, minimizing age variance in the resulting chronogram.
...
The BLADJ algorithm requires a root age \DIFdelbegin \DIFdel{estimate }\DIFdelend to run. \DIFdelbegin \DIFdel{If }\DIFdelend \DIFaddbegin \DIFadd{Users can provide an appropriate root age estimate of their own or one obtained from the literature. If a root age is not provided and }\DIFaddend there is no information on the age of the root in the chronogram database, \DIFdelbegin \DIFdel{users can provide an estimate from the literature. If none is provided, DateLife will choose an }\DIFdelend \DIFaddbegin \DIFadd{DateLife chooses a random }\DIFaddend age for the root\DIFdelbegin \DIFdel{so that it can return a dated topology. It will also }\DIFdelend \DIFaddbegin \DIFadd{, so that a dated tree topology can be generated with BLADJ. In this case, DateLife will }\DIFaddend provide a conspicuous warning message\DIFaddbegin \DIFadd{, }\DIFaddend so that users are aware that the root of the chronogram \DIFdelbegin \DIFdel{does not have information available }\DIFdelend \DIFaddbegin \DIFadd{was chosen at random because there was no information available for it }\DIFaddend in the chronogram database, along with suggestions on how the user can \DIFaddbegin \DIFadd{find and }\DIFaddend provide an appropriate age for the root \DIFaddbegin \DIFadd{of the initial topology}\DIFaddend .

\DIFdelbegin \DIFdel{In the absence of genetic data, BLADJ is a very agnostic way to assign ages to nodes with no available data, as it does not require any assumptions on the underlying model of branch length distribution. It is however }\DIFdelend \DIFaddbegin \DIFadd{An alternative to BLADJ to date tree topologies in the absence of initial branch lengths that is }\DIFaddend common practice in the literature \DIFaddbegin \DIFadd{is }\DIFaddend to use a birth-death model to \DIFdelbegin \DIFdel{assign ages to nodes with no genetic data }\DIFdelend \DIFaddbegin \DIFadd{draw branch lengths }\DIFaddend (Jetz, Thomas, Joy, Hartmann, \& Mooers, 2012; Rabosky et al., 2018; Smith \& Brown, 2018). \DIFdelbegin \DIFdel{To do so, DateLife implements }\DIFdelend \DIFaddbegin \DIFadd{In addition to the initial tree topology and nodes with age data, these methods require initial values of speciation and extinction rate parameters provided by the user. DateLife implements this approach with }\DIFaddend MrBayes (Huelsenbeck \& Ronquist, 2001; Ronquist \& Huelsenbeck, 2003), using nodes with published age data as calibration priors on \DIFdelbegin \DIFdel{a fixed topology }\DIFdelend \DIFaddbegin \DIFadd{nodes of a tree topology with no branch lengths}\DIFaddend , a simple birth-death model with \DIFdelbegin \DIFdel{parameters that can be determined }\DIFdelend \DIFaddbegin \DIFadd{speciation and extinction rate parameters that are provided }\DIFaddend by the user, and no genetic data. \DIFaddbegin \DIFadd{However, BLADJ is the default option in DateLife, as it does not require any information on diversification rates for the phylogenetic sample to draw from a branch length distribution.
}\DIFaddend 
...
While using initial branch length data is the golden standard for phylogenetic dating analyses, \DIFdelbegin \DIFdel{estimating trees with branch lengths proportional to substitution rates per site }\DIFdelend \DIFaddbegin \DIFadd{obtaining such information from scratch is not an easy task: it }\DIFaddend requires obtaining primary data, assembling and curating a homology \DIFaddbegin \DIFadd{(orthology) }\DIFaddend hypothesis, and choosing and implementing a method for phylogenetic inference.
...
Once names have been processed as described in methods, a name search through the chronogram database can be performed in less than a minute, even with a very large number of taxon names (Fig. \DIFdelbegin \DIFdel{2}\DIFdelend \DIFaddbegin \DIFadd{1}\DIFaddend ).
...
We illustrate the DateLife workflow using a family within the passeriform birds encompassing the true finches, Fringillidae, as case study. On a small example, we analysed 6 bird species, and results from each step of the workflow are shown in Figure \DIFdelbegin \DIFdel{3. }\DIFdelend \DIFaddbegin \DIFadd{2. }\DIFaddend As a second example, we analysed 289 bird species in the family Fringillidae that are included in the NCBI taxonomy. \DIFdelbegin \DIFdel{The summary chronogram resulting }\DIFdelend \DIFaddbegin \DIFadd{One clade from the full summary chronogram result }\DIFaddend from the DateLife analysis is shown \DIFdelbegin \DIFdel{in Figure 5, and }\DIFdelend \DIFaddbegin \DIFadd{Figure 3. The full chronogram for all 289 species and the }\DIFaddend results from previous steps of the workflow are available as Supplementary Figures.
...
Discovering this synonym allowed assigning five age data points for the parent node of \emph{Emberiza elegans}, shown as \emph{Schoeniclus elegans} in Figure \DIFdelbegin \DIFdel{3a}\DIFdelend \DIFaddbegin \DIFadd{2}\DIFaddend , which would not have had any data otherwise.

\textbf{\emph{Searching the database.--}}
DateLife used the processed input names to search the local chronogram database and found 9 matching chronograms from 6 different studies (Fig. \DIFdelbegin \DIFdel{3b}\DIFdelend \DIFaddbegin \DIFadd{2c}\DIFaddend ). Three studies matched five input names (Barker, Burns, Klicka, Lanyon, \& Lovette, 2015; Hedges, Marin, Suleski, Paymer, \& Kumar, 2015; Jetz et al., 2012), one study matched four input names (Hooper \& Price, 2017) and two studies matched two input names (Barker, Burns, Klicka, Lanyon, \& Lovette, 2013; Burns et al., 2014). No studies matched all input names. Together, source chronograms provide 28 unique age data points, covering all nodes on our chosen tree topology to date (Table 1).

\textbf{\emph{Summarizing search results.--}}
DateLife obtained OpenTree's synthetic tree topology for these taxa (Fig. \DIFdelbegin \DIFdel{3c}\DIFdelend \DIFaddbegin \DIFadd{2d}\DIFaddend ), and congruified and mapped age data to nodes in this chosen topology, shown in Table 1.
The name processing step allowed including five data points for node \enquote{n4} (parent of \emph{Schoeniclus elegans}\DIFdelbegin \DIFdel{; Fig. 3A}\DIFdelend ) that would not have had any data otherwise due to name mismatch.
...
Median summary age data for node \enquote{n2} was excluded as final calibration because it is older than the median age of a more inclusive node, \enquote{n1} (Fig. \DIFdelbegin \DIFdel{3c4}\DIFdelend \DIFaddbegin \DIFadd{2g}\DIFaddend ).
...
According to OpenTree's synthetic tree, species belonging to the family Fringillidae do not form a monophyletic group (\DIFdelbegin \DIFdel{Fig. 4a}\DIFdelend \DIFaddbegin \DIFadd{Supplementary Fig. S1}\DIFaddend ). Hence, a topology containing only the 289 species from the original query was extracted from Open Tree of Life's synthetic tree v12.3 (\DIFdelbegin \DIFdel{Fig. 4b}\DIFdelend \DIFaddbegin \DIFadd{Supplementary Fig. S2}\DIFaddend ; Open Tree Of Life et al., 2019).

\DIFdelbegin \DIFdel{Source }\DIFdelend \DIFaddbegin \DIFadd{All 19 source }\DIFaddend chronograms (Supplementary Figs. \DIFdelbegin \DIFdel{S2-S20}\DIFdelend \DIFaddbegin \DIFadd{S5-S23}\DIFaddend ) were congruified to OpenTree's topology shown in \DIFdelbegin \DIFdel{Figure 4b}\DIFdelend \DIFaddbegin \DIFadd{Supplementary Figure S2}\DIFaddend , reducing the original 1,206 node age data set to 818 different data points (Supplementary Table S2) that \DIFdelbegin \DIFdel{can }\DIFdelend \DIFaddbegin \DIFadd{could }\DIFaddend be used as calibrations for \DIFdelbegin \DIFdel{the chosen topology(Fig. 4b). }\DIFdelend \DIFaddbegin \DIFadd{that chosen topology. }\DIFaddend The congruent node age data points were summarized for each node, resulting in 194 summary node ages. From these 21 were excluded as secondary calibrations because they were older than the ancestral node. The remaining 173 summary node ages were used as secondary calibrations to obtain a fully dated (and resolved) phylogeny with the program BLADJ (\DIFdelbegin \DIFdel{Fig. 5). }\DIFdelend \DIFaddbegin \DIFadd{Supplementary Figure S3). Results for a subgroup are shown in Figure 3.
}\DIFaddend 
...
We performed a cross validation test of a DateLife analysis using the Fringillidae source chronograms obtained above (Supplementary Figs. \DIFdelbegin \DIFdel{S2-S20).
We used as }\DIFdelend \DIFaddbegin \DIFadd{S5-S23).
As }\DIFaddend inputs for a DateLife analysis\DIFaddbegin \DIFadd{, we used }\DIFaddend all individual tree topologies from each of the 19 source chronograms from 13 studies, treating their node ages as unknown.
...
We found that node ages from the original studies, and ages estimated using all other age data available are \DIFdelbegin \DIFdel{largely }\DIFdelend \DIFaddbegin \DIFadd{generally }\DIFaddend correlated (Fig. \DIFdelbegin \DIFdel{6}\DIFdelend \DIFaddbegin \DIFadd{4}\DIFaddend ).
For five studies, DateLife tended to underestimate ages for topologically deeper nodes (those with many descendant taxa, aka \enquote{closer to the root}) relative to the original estimate, and overestimate ages for nodes closer to the tips. Accordingly, root ages are generally older in the original study than estimated using cross-validated ages (Supplementary Fig. \DIFdelbegin \DIFdel{S1}\DIFdelend \DIFaddbegin \DIFadd{S4}\DIFaddend ).
...
DateLife's goal is to improve availability\DIFdelbegin \DIFdel{and accessibility}\DIFdelend \DIFaddbegin \DIFadd{, accessibility, and reusability }\DIFaddend of state-of-the-art data on evolutionary time frame of organisms, to allow users from all areas of science and with all levels of expertise to compare, \DIFdelbegin \DIFdel{reuse, }\DIFdelend \DIFaddbegin \DIFadd{use }\DIFaddend and reanalyse expert age data for their own applications. As such, it is designed as an open service that does not require any expert biological knowledge --besides the scientific names of the species or group \DIFaddbegin \DIFadd{that }\DIFaddend users want to work \DIFdelbegin \DIFdel{with, }\DIFdelend \DIFaddbegin \DIFadd{with-- }\DIFaddend to use any of its functionalities.
...
We would like to emphasize on the importance of sharing chronogram data, including systematically curated metadata, into open repositories, such as OpenTree's Phylesystem (McTavish et al., 2015) for the benefit of \DIFaddbegin \DIFadd{research and }\DIFaddend the scientific community as a whole.
\DIFaddbegin \DIFadd{Another important source of expert data on time of lineage divergence is TimeTree's database (Hedges, Dudley, \& Kumar, 2006), which holds chronograms from more than 4k published studies, and is fully browsable using its graphical user interface (timetree.org). TimeTree's chronogram database was not accessible in computer readable format until very recently (Kumar et al., 2022), when its terms of use and website application were updated, now allowing some kinds of reuse, but not redistribution.}\\
 \DIFadd{The inaccessibility of TimeTree's database was an inspiration for the DateLife project, which was born as a prototype tool initially developed over a series of hackathons at the National Evolutionary Synthesis Center, NC, USA (Stoltzfus et al., 2013), as the need to make scientific information that is funded by the public practically available to the public was acknowledged and prioritized.
}\DIFaddend 
...
Using several secondary calibrations (as opposed to just one) can provide sufficient information to alleviate or even neutralize potential biases (Graur \& Martin, 2004; Sauquet, 2013; Shaul \& Graur, 2002). Our cross validation analysis also provides insight into the application of secondary calibrations. Node ages summarized with DateLife and those from the original studies are well correlated (Supplementary Figs. \DIFdelbegin \DIFdel{S2-S20}\DIFdelend \DIFaddbegin \DIFadd{S5-S23}\DIFaddend ). We also note that DateLife estimates for nodes closer to the root tend to be slightly younger than ages from the original studies. In contrast, nodes closer to the tips tend to be slightly older when estimated using our secondary calibrations than ages from the original studies. The only exception to this trend was observed in Burns et al. (2014) chronogram, which generally displays much younger node ages when estimated using secondary calibrations than the original study (Supplementary Figs. \DIFdelbegin \DIFdel{S1, S5}\DIFdelend \DIFaddbegin \DIFadd{S4c, S10}\DIFaddend ), supporting previous observations (Sauquet et al., 2012; Schenk, 2016). However, these younger dates are more likely an example of how multiple secondary calibrations can correct erroneous estimates, as dates on the Burns et al. (2014) tree were obtained using a single secondary calibration based on a previously estimated molecular evolution rate across birds from Weir and Schluter (2008), and appear as major outliers compared to alternate estimates for the same nodes based on primary fossil calibrations (Fig. \DIFdelbegin \DIFdel{5).
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Further research is needed to fully understand the effects of using secondary calibrations and the use of resulting chronograms in downstream analyses (Hipsley \& Müller, 2014; Powell et al. , 2020; Schenk, 2016; Shaul \& Graur, 2002}\DIFdelend \DIFaddbegin \DIFadd{3, Supplementary Fig. S3}\DIFaddend ).
...
The exercise of summarizing age data from across multiple studies \DIFaddbegin \DIFadd{is a common resource in research, as it }\DIFaddend provides the opportunity to work with a chronogram that reflects a unified evolutionary history for a lineage, by putting together evidence from different hypotheses. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{For example, the }\DIFaddend largest, and taxonomically broadest chronogram currently available from OpenTree was constructed summarizing age data from 2,274 published chronograms using NCBI's taxonomic tree as backbone (Hedges et al., 2015)\DIFdelbegin \DIFdel{.
A summarizing exercise may also }\DIFdelend \DIFaddbegin \DIFadd{, which has been widely reused for research.
Finally, we note that summarizing chronograms should be done with caution, as it may }\DIFaddend amplify the effect of uncertainty and errors in source data, and blur parts of the evolutionary history of a lineage that might only be reflected in source chronograms and lost on the summary chronogram (Sauquet et al., 2021).
...
DateLife provides a straightforward way to get an informed picture of the state of knowledge \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend the time frame of evolution of different regions of the tree of life, and allows identifying regions that require more research, or that have conflicting information.
...
Supplementary \DIFdelbegin \DIFdel{Figures can be viewed and downloaded from their Zenodo repository (}%DIFDELCMD < \url{https://doi.org/10.5281/zenodo.6683667}%%%
\DIFdel{).
Supplementary }\DIFdelend material, including \DIFaddbegin \DIFadd{figures, tables, }\DIFaddend code, biological examples, benchmark results, data files and online-only appendices, can be \DIFaddbegin \DIFadd{viewed and }\DIFaddend downloaded from the Dryad data repository (\url{https://doi.org/10.5061/dryad.cnp5hqc6w}), as well as \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{from }\DIFaddend the Zenodo stable repositories that host the reproducible manuscript (\url{https://doi.org/10.5281/zenodo.7435094}), the biological examples (\url{https://doi.org/10.5281/zenodo.7435101}), \DIFdelbegin \DIFdel{and }\DIFdelend the software benchmark (\url{https://doi.org/10.5281/zenodo.7435106})\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, and the figures (}\url{https://doi.org/10.5281/zenodo.6683667}\DIFadd{).
Dryad's data publication fee is covered by the Society of Systematic Biologists.
}\DIFaddend Development versions corresponding to all of the above are hosted on GitHub, \DIFdelbegin \DIFdel{accesible }\DIFdelend \DIFaddbegin \DIFadd{accessible }\DIFaddend at \url{https://github.com/LunaSare/datelifeMS1}, \url{https://github.com/LunaSare/datelife_examples}, and \url{https://github.com/LunaSare/datelife_benchmark}.
...
\DIFdelbegin \DIFdel{The DateLife project was born as a prototype tool aiming to provide the services describe in this paper, and was initially developed over a series of hackathons at the National Evolutionary Synthesis Center, NC, USA (Stoltzfus et al., 2013).
}\DIFdelend We thank colleagues from the O'Meara Lab at the University
...
\leavevmode\DIFaddbegin \hypertarget{ref-Hedges2006}{}%DIF > 
\DIFadd{Hedges, S. B., Dudley, J., \& Kumar, S. (2006). TimeTree: A public knowledge-base of divergence times among organisms. }\emph{\DIFadd{Bioinformatics}}\DIFadd{, }\emph{22}(23)\DIFadd{, 2971--2972. }\url{https://doi.org/10.1093/bioinformatics/btl505}
...
\leavevmode\DIFaddbegin \hypertarget{ref-kumar2022timetree}{}%DIF > 
\DIFadd{Kumar, S., Suleski, M., Craig, J. M., Kasprowicz, A. E., Sanderford, M., Li, M., \ldots{} Hedges, S. B. (2022). TimeTree 5: An expanded resource for species divergence times. }\emph{\DIFadd{Molecular Biology and Evolution}}\DIFadd{, }\emph{39}(8)\DIFadd{, msac174.
}

\leavevmode\DIFaddend \hypertarget{ref-lamichhaney2015evolution}{}%
