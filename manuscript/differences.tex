...
%DIF DEL manuscript-1st-submission.tex   Mon Jan 23 16:21:07 2023
%DIF ADD manuscript.tex                  Mon Feb 20 23:06:07 2023
...
{}{\PatchFailed}
%DIF 139c139
%DIF < \keywords{Tree; Phylogeny; Scaling; Dating; Ages; Divergence times; Open Science; Congruification; Supertree; Calibrations; Secondary calibrations\newline\indent Word count: 4959}
%DIF -------
\keywords{Tree; Phylogeny; Scaling; Dating; Ages; Divergence times; Open Science; Congruification; Supertree; Calibrations; Secondary calibrations\newline\indent Word count: 5605} %DIF > 
%DIF -------
...
\abstract{%
%DIF 180-186c180-187
%DIF < Achieving a high-quality reconstruction of a phylogenetic tree with branch lengths proportional to absolute time (chronogram) is a difficult and time-consuming task. But the increased availability of fossil and molecular data, and time-efficient analytical techniques has resulted in many recent publications of large chronograms for a large number and wide diversity of organisms.
%DIF < Knowledge of the evolutionary time frame of organisms is key for research in the natural sciences. It also represent valuable information for education, science communication, and policy decisions.
%DIF < When chronograms are shared in public and open databases, this wealth of expertly-curated and peer-reviewed data on evolutionary timeframe is exposed in a programatic and reusable way, as intensive and localized efforts have improved data sharing practices, as well as incentivizited open science in biology.
%DIF < Here we present DateLife, a service implemented as an R package and an R Shiny website application available at www.datelife.org, that provides functionalities for efficient and easy finding, summary, reuse, and reanalysis of expert, peer-reviewed, public data on time frame of evolution.
%DIF < The main DateLife workflow constructs a chronogram for any given combination of taxon names by searching a local chronogram database constructed and curated from the Open Tree of Life Phylesystem phylogenetic database, which incorporates phylogenetic data from the TreeBASE database as well.
%DIF < We implement and test methods for summarizing time data from multiple source chronograms using supertree and congruification algorithms, and using age data extracted from source chronograms as secondary calibration points to add branch lengths proportional to absolute time to a tree topology.
%DIF < DateLife will be useful to increase awareness of the existing variation in alternative hypothesis of evolutionary time for the same organisms, and can foster exploration of the effect of alternative evolutionary timing hypotheses on the results of downstream analyses, providing a framework for a more informed interpretation of evolutionary results.
%DIF -------
Chronograms --phylogenies with branch lengths proportional to time-- represent key data on evolutionary time frame for the study of natural processes in many areas of biological research. %DIF > 
Chronograms also provide valuable information that can be used for education, science communication, and policy decisions. %DIF > 
Yet, achieving a high-quality reconstruction of a chronogram is a difficult and resource-consuming task. %DIF > 
Here we present DateLife, a service implemented as an R package and an R Shiny web application available at www.datelife.org, that provides services for efficient and easy discovery, summary, reuse, and reanalysis of node age data mined from a curated database of expert, peer-reviewed, and openly available chronograms. %DIF > 
The main DateLife workflow starts with one or more scientific taxon names provided by a user. Names are processed and standardized to a unified taxonomy. Then, DateLife runs a name match across its local chronogram database that is curated from Open Tree of Life's phylogenetic repository, and extracts all chronograms that contain at least two queried taxon names, along with their metadata. %DIF > 
Finally, node ages from matching chronograms are mapped upon a chosen tree topology using the congruification algorithm. Congruified node ages are used as secondary calibrations to date the chosen topology, with or without initial branch lengths, using different phylogenetic dating methods such as BLADJ, treePL, PATHd8 and MrBayes. %DIF > 
We performed a cross-validation test to compare node ages resulting from a DateLife analysis to those from the original chronograms (i.e, obtained with primary calibrations), and found that DateLife's node age estimates appear generally correlated to those from the original chronograms, with the largest variation in ages occurring around topologically deeper nodes. %DIF > 
Software such as DateLife can help increase awareness of the existing variation in alternative hypothesis of evolutionary time frame for the same organisms, and can support exploration of the effect of alternative chronogram hypotheses on downstream analyses, providing a framework for a more informed interpretation of evolutionary results. %DIF > 
%DIF -------
}
...
\DIFdelbegin %DIFDELCMD < \begin{minipage}[t]{17.3cm}
...
%DIFDELCMD < %%%
\DIFdelend \hypertarget{creating-a-search-query}{%
\subsection{Creating a search query}\label{creating-a-search-query}}

DateLife starts by processing an input consisting of at least \DIFdelbegin \DIFdel{two taxon names, which }\DIFdelend \DIFaddbegin \DIFadd{one taxon scientific name. Multiple input names }\DIFaddend can be provided as a comma separated character string or as tip labels on a tree.
...
\DIFdelbegin \DIFdel{If the flag is }\DIFdelend \DIFaddbegin 

\DIFadd{Besides binomial species names, DateLife accepts scientific names from any inclusive taxonomic group (e.g., genus, family, tribe), as well as subspecific taxonomic variants (e.g., subspecies, variants, strains). If a taxon name belongs to an inclusive taxonomic group, DateLife has two alternative behaviors defined by the }\enquote{get species from taxon} \DIFadd{flag. If the flag is active, DateLife retrieves all species names within the taxonomic group from the standard taxonomy of choice, and adds them to the search query. In this case, subspecific variants are excluded.
If the flag is }\DIFaddend inactive, DateLife excludes any taxon names above the species level from the search query.
...
%DIFDELCMD < %%%
\DIFdel{The processed input }\DIFdelend \DIFaddbegin \DIFadd{Species and subspecific variant names are processed and searched as provided by the user.
...
(\DIFdelbegin \DIFdel{Jun 22, 2022}\DIFdelend \DIFaddbegin \DIFadd{Feb 20, 2023}\DIFaddend ),
...
\DIFdelbegin \emph{\textbf{\DIFdel{Choosing a topology.--}}%DIFAUXCMD
}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \hypertarget{choosing-a-topology}{%
\subsection{Choosing a topology}\label{choosing-a-topology}}

\DIFaddend DateLife requires a tree topology to summarize age data upon.
...
\DIFdelbegin \emph{\textbf{\DIFdel{Dating the topology.--}}%DIFAUXCMD
}
%DIFAUXCMD
\DIFdel{Input topologies from OpenTree or the supertreeapproach described above do not include branch length estimates of any kind.
...
}\DIFdelend \DIFaddbegin \hypertarget{applying-secondary-calibrations}{%
...
\DIFdelbegin \DIFdel{By default, DateLife implements the }\DIFdelend \DIFaddbegin \hypertarget{dating-a-tree-topology}{%
...
This minimizes }\DIFdelend \DIFaddbegin \DIFadd{calibrated nodes, minimizing }\DIFaddend age variance in the resulting chronogram (Webb et al., 2008)\DIFaddbegin \DIFadd{, which has proven useful for ecological analyses that require age data (Webb et al., 2008)}\DIFaddend .
...
\DIFdelbegin %DIFDELCMD < \includegraphics{../figures/fig_runtime_main.pdf}
...
%DIFDELCMD < %%%
\DIFdelend \texttt{datelife}'s code performance was evaluated with a set of unit tests designed and
...
%DIFDELCMD < %%%
\DIFdelend \hypertarget{searching-the-database}{%
...
%DIFDELCMD < %%%
\DIFdelend \hypertarget{summarizing-search-results-1}{%
...
%DIFDELCMD < %%%
\DIFdelend \hypertarget{an-example-with-the-family-of-true-finches}{%
...
%DIFDELCMD < %%%
\DIFdelend Age data from source chronograms was congruified to OpenTree's topology (Fig. \ref{fig:fringillidae-topologies}B), reducing the age data set to 818 different data points (\DIFdelbegin \DIFdel{Supplementray }\DIFdelend \DIFaddbegin \DIFadd{Supplementary }\DIFaddend Table S2). For each congruent node, age summary statistics were calculated and used as fixed secondary calibrations over the chosen tree topology, to obtain a fully dated phylogeny with the program BLADJ (Fig. \ref{fig:fringillidages}).

\DIFdelbegin %DIFDELCMD < \begin{minipage}{18cm}
...
%DIFDELCMD < %%%
\DIFdelend \hypertarget{cross-validation-test}{%
...
For five studies, \DIFdelbegin \DIFdel{Datelife }\DIFdelend \DIFaddbegin \DIFadd{DateLife }\DIFaddend tended to underestimate ages for topologically deeper nodes (those with many descendant taxa, aka \enquote{closer to the root}) relative to the original estimate, and overestimate ages for nodes closer to the tips.
...
\DIFdelbegin %DIFDELCMD < \hypertarget{primary-vs-secondary-calibrations}{%
%DIFDELCMD < \subsubsection{Primary vs Secondary calibrations}\label{primary-vs-secondary-calibrations}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{primary-vs-secondary-calibrations}{%
...
\DIFdelbegin %DIFDELCMD < \hypertarget{effects-on-downstream-analyses}{%
%DIFDELCMD < \subsection{Effects on downstream analyses}\label{effects-on-downstream-analyses}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{effects-of-phylogenetic-sampling-on-downstream-analyses}{%
...
}\DIFaddend Hence, DateLife's workflow features different ways of \DIFdelbegin \DIFdel{estimating }\DIFdelend \DIFaddbegin \DIFadd{assigning }\DIFaddend node ages in the absence of calibrations and branch length information for certain taxa.
\DIFdelbegin %DIFDELCMD < \enquote{Making up} %%%
\DIFdel{branch lengths }\DIFdelend \DIFaddbegin \DIFadd{Completing a phylogeny using a stochastic birth-death politomy resolver and a backbone taxonomy }\DIFaddend is a common practice in scientific publications: Jetz et al. (2012), created a chronogram of all 9, 993 bird species, where 67\% had molecular data and the rest was simulated; Rabosky et al. (2018) created a chronogram of 31, 536 ray-finned fishes, of which only 37\% had molecular data; Smith and Brown (2018) constructed a chronogram of 353, 185 seed plants where only 23\% had molecular data. \DIFaddbegin \DIFadd{These stochastically resolved chronograms provide less biased diversification rates estimated with methods that account for sampling fractions (Chang et al., 2020; Cusimano et al., 2012), but can also introduce spurious patterns of early bursts of diversification (Cusimano \& Renner, 2010; Sun et al., 2020).
}\DIFaddend 

\DIFdelbegin \DIFdel{Notably, }\DIFdelend \DIFaddbegin \DIFadd{Taxonomy based politomy resolvers also introduce topological differences.
...
\DIFadd{While stochastic politomy resolvers have been useful to advance research, notably, }\DIFaddend risks come with this practice.
...
We urge DateLife users }\DIFaddend to follow the example of the large tree papers cited above, by carefully considering the statistical assumptions being made, \DIFaddbegin \DIFadd{potential biases, }\DIFaddend and assessing the consistency of the results with prior work.
...
\DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{downloadable from Zenodo (}\url{https://doi.org/10.5281/zenodo.593938}\DIFadd{) and the CRAN repository (Sanchez-Reyes et al., 2022).
}\DIFaddend DateLife's web application is maintained using RStudio's shiny server and the shiny package open infrastructure, as well as Docker and OpenTree's infrastructure (dates.opentreeoflife.org/datelife).
\texttt{datelife}'s \DIFdelbegin \DIFdel{R package }\DIFdelend stable version is available for installation from \DIFdelbegin \DIFdel{the CRAN repository (}%DIFDELCMD < \url{https://cran.r-project.org/package=datelife}%%%
\DIFdel{) }\DIFdelend \DIFaddbegin \DIFadd{CRAN repository }\DIFaddend using the command \texttt{install.packages(pkgs\ =\ "datelife")} from within R.
...
\DIFdelbegin \DIFdel{Code used to generate all versions of this manuscript, the }\DIFdelend \DIFaddbegin \DIFadd{Supplementary material, including code, }\DIFaddend biological examples, \DIFaddbegin \DIFadd{benchmark results, data files and online-only appendices, can be found in the Dryad data repository (}\url{https://doi.org/10.5061/dryad.cnp5hqc6w}\DIFadd{), }\DIFaddend as well as \DIFdelbegin \DIFdel{the benchmark of functionalities are available at }\href{https://github.com/LunaSare/datelifeMS1}{\DIFdel{datelifeMS1}}%DIFAUXCMD
...
\DIFdel{repositories in LLSR's GitHub account}\DIFdelend \DIFaddbegin \DIFadd{in the Zenodo stable repositories that host the reproducible manuscript (}\url{https://doi.org/10.5281/zenodo.7435094}\DIFadd{), the biological examples (}\url{https://doi.org/10.5281/zenodo.7435101}\DIFadd{), and the software benchmark (}\url{https://doi.org/10.5281/zenodo.7435106}\DIFadd{). The corresponding development versions hosted on GitHub can be found at }\url{https://github.com/LunaSare/datelifeMS1\#readme}\DIFadd{, }\url{https://github.com/LunaSare/datelife_examples\#readme}\DIFadd{, and }\url{https://github.com/LunaSare/datelife_benchmark\#readme}\DIFaddend .
...
The DateLife project was born as a prototype tool aiming to provide \DIFdelbegin \DIFdel{these services }\DIFdelend \DIFaddbegin \DIFadd{the services describe in this paper}\DIFaddend , and was initially developed over a series of hackathons at the National Evolutionary Synthesis Center, NC, USA (Stoltzfus et al., 2013).
...
\leavevmode\DIFaddbegin \hypertarget{ref-bodenhofer2015msa}{}%DIF > 
...
\leavevmode\DIFdelbegin %DIFDELCMD < \hypertarget{ref-datelifeR}{}%%%
\DIFdelend \DIFaddbegin \hypertarget{ref-sanchez2022}{}\DIFaddend %
Sanchez-Reyes, L. L., O'Meara, B., Eastman, J., Heath, T., Wright, A., Schliep, K., \ldots{} Alfaro, M. (2022). \texttt{datelife}: Scientific Data on Time of Lineage Divergence for Your Taxa. \DIFaddbegin \DIFadd{In }\DIFaddend \emph{R \DIFdelbegin \DIFdel{Package Release V0}\DIFdelend \DIFaddbegin \DIFadd{package version 0.6}\DIFaddend .6\DIFdelbegin \DIFdel{.2}\DIFdelend }. Retrieved from \DIFdelbegin %DIFDELCMD < \url{https://doi.org/10.5281/zenodo.593938}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \href{https://CRAN.R-project.org/package=datelife\%20and\%20https://doi.org/10.5281/zenodo.593938}{\DIFadd{https://CRAN.R-project.org/package=datelife and https://doi.org/10.5281/zenodo.593938}}
\DIFaddend 
