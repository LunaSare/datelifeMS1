...
%DIF ADD manuscript.tex                  Mon Feb 13 11:01:42 2023
...
A unique feature of the Phylesystem is that any user can add new published, state-of-the-art chronograms any time, through their curator application (\url{https://tree.opentreeoflife.org/curator}). As chronograms are added to Phylesystem, they \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{can be }\DIFaddend incorporated into the chronogram database of the \texttt{datelife} package. \DIFaddbegin \texttt{\DIFadd{datelife}}\DIFadd{'s chronogram database is currently manually updated as new chronogram data is added to Phylesystem. }\DIFaddend The updated database is assigned a new version number, followed by a package release on CRAN.
\DIFdelbegin \texttt{\DIFdel{datelife}}%DIFAUXCMD
\DIFdel{'s chronogram database is updated as new chronogram data is added to Phylesystem, at a minimum of once a month and a maximum of every 6 months.
}\DIFdelend Users can also implement functions from the \texttt{datelife} R package to trigger an update of the local chronogram database, to incorporate any new chronograms to the user's DateLife analysis before an official database update is released on CRAN.
...
\DIFaddbegin \emph{\textbf{\DIFadd{Dating a tree topology.--}}}
\DIFaddend By default, DateLife implements the Branch Length Adjuster (BLADJ\DIFaddbegin \DIFadd{; Webb et al., 2008; Webb \& Donoghue, 2005}\DIFaddend ) algorithm to obtain a fully dated topology.
BLADJ \DIFdelbegin \DIFdel{fixes node ages that have }\DIFdelend \DIFaddbegin \DIFadd{takes an initial topology and ignores branch length data if present. Next, it fixes ages on nodes with }\DIFaddend calibration data, and distributes time \DIFdelbegin \DIFdel{between }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend nodes with no data evenly between \DIFdelbegin \DIFdel{nodes with calibration data.
This minimizes }\DIFdelend \DIFaddbegin \DIFadd{calibrated nodes, minimizing }\DIFaddend age variance in the resulting chronogram (Webb et al., 2008). \DIFdelbegin \DIFdel{BLADJ does not use branch lengths even when they are present in the input tree or summarizing topology.}\DIFdelend \DIFaddbegin \DIFadd{This method is useful for ecological analyses (Webb et al., 2008).
}\DIFaddend When there is conflict \DIFdelbegin \DIFdel{in ages between nodes with calibration data, BLADJ }\DIFdelend \DIFaddbegin \DIFadd{between ages of calibrated nodes, }\DIFaddend ignores node ages that are older than the age of a parent node.
BLADJ requires a root age estimate. If there is no information on the age of the root in the chronogram database, users can provide an estimate from the literature. If none is provided, DateLife \DIFdelbegin \DIFdel{assigns an arbitrary age to the root as 10\% older than the oldest age available within the group}\DIFdelend \DIFaddbegin \DIFadd{will not return a dated topology and will provide a warning message along with suggestions on how the user can provide an age for the root so the dating analysis can be finished.
}

\DIFadd{Alternatively, the user can choose to obtain a fully dated phylogeny
MrBayes (Huelsenbeck \& Ronquist, 2001; Ronquist \& Huelsenbeck, 2003) approach in DateLife uses the calibrations as priors on node ages}\DIFaddend .

\DIFdelbegin \DIFdel{Alternative }\DIFdelend \DIFaddbegin \DIFadd{The latter methods can assign dates using a birth-death model (all of them????), they require initial branch lengths and thus require more time and expertise to run, which makes BLADJ the fastest and most practical method to obtain a dated tree.
}

\DIFadd{In the case the user can provide or obtain branch length data along with a tree topology, }\DIFaddend phylogenetic dating options supported in DateLife \DIFdelbegin \DIFdel{(MrBayes, PATHD8, TreePL) }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend incorporate branch length information from the input topology in combination with the \DIFdelbegin \DIFdel{calibrations.
}\DIFdelend \DIFaddbegin \DIFadd{secondary calibrations:
}\DIFaddend PATHd8 is a non-clock, rate-smoothing method (Britton et al., 2007) to date trees\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{;
}\DIFaddend treePL (Smith \& O'Meara, 2012), is a semi-parametric, rate-smoothing, penalized likelihood dating method (Sanderson, 2002).
\DIFdelbegin \DIFdel{The MrBayes (Huelsenbeck \& Ronquist, 2001; Ronquist \& Huelsenbeck, 2003)approach in DateLife uses the calibrations as priors on node ages. }\DIFdelend

\DIFaddbegin \emph{\textbf{\DIFadd{Dating a tree with branch lengths--.}}}
\DIFadd{Topologies obtained from OpenTree and with the supertree approach described above lack branch length data.
...
\DIFadd{A fast solution implemented in DateLife is as follows.
...
\DIFadd{The BOLD sequence alignment is then used to obtain initial branch lengths with the accelerated transformation (ACCTRAN) parsimony algorithm, which resolves ambiguous character optimization, by assigning changes along branches of the tree as close to the root as possible (Agnarsson \& Miller, 2008). This results in older nodes (Forest et al., 2005).
Optionally, the likelihood of the tree topology, the alignment and the reconstructed branch lengths given different evolutionary models, is computed using functions from the }\texttt{\DIFadd{phangorn}} \DIFadd{package (Schliep, 2011).
}

\DIFadd{Relative branch length information provides key data for phylogenetic dating, especially for nodes without secondary calibrations available. Yet, topologies without branch lengths can also be dated.
}

\DIFaddend \emph{\textbf{Visualizing results.--}}
Finally, users can save all source and summary chronograms in formats that permit reuse and \DIFdelbegin \DIFdel{reanalyses (newick and }\DIFdelend \DIFaddbegin \DIFadd{reanalysis (such as newick and the }\DIFaddend R \enquote{phylo} format), as well as visualize and compare results graphically, or construct their own graphs using DateLife's chronogram plot generation functions available from the R package \texttt{datelifeplot} (Sanchez-Reyes \& O'Meara, 2022).
...
Once identified, DateLife pruned \DIFaddbegin \DIFadd{these }\DIFaddend matching chronograms to \DIFdelbegin \DIFdel{keep Fringillidae species names on tips only}\DIFdelend \DIFaddbegin \DIFadd{remove tips that do not belong to the queried taxon names}\DIFaddend , and transformed these pruned chronograms to pairwise distance matrices, revealing 1, 206 different age data points available for species within the Fringillidae (Supplementray Table S1).
...
\DIFdelbegin \DIFdel{Code used to generate all versions of this manuscript, the biological examples , }\DIFdelend \DIFaddbegin \DIFadd{Supplementary material, including code, biological examples and benchmark results data files and online-only appendices, can be found in the Dryad data repository (}\url{https://doi.org/10.5061/dryad.cnp5hqc6w}\DIFadd{), }\DIFaddend as well as \DIFdelbegin \DIFdel{the benchmark of functionalities are available at }\href{https://github.com/LunaSare/datelifeMS1}{\DIFdel{datelifeMS1}}%DIFAUXCMD
...
\DIFdel{repositories in LLSR's GitHub account}\DIFdelend \DIFaddbegin \DIFadd{in the GitHub repositories used to develop the reproducible manuscript (}\url{https://doi.org/10.5281/zenodo.7435094}\DIFadd{), the biological examples (}\url{https://doi.org/10.5281/zenodo.7435101}\DIFadd{), and the software benchmark (}\url{https://doi.org/10.5281/zenodo.7435106}\DIFadd{)}\DIFaddend .

\hypertarget{funding}{%
\section{Funding}\label{funding}}

Funding was provided by the US National Science Foundation (NSF) grants ABI-1458603 to the Datelife project; DBI-0905606 to the National Evolutionary Synthesis Center (NESCent)\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend ABI-1458572 to the Phylotastic project\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend and ABI-1759846 to the Open Tree of Life project.
