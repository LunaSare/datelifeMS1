...
%DIF ADD manuscript.tex                  Mon Jan 23 16:21:21 2023
...
BLADJ \DIFaddbegin \DIFadd{takes an initial topologies without any branch length data, }\DIFaddend fixes node ages that have calibration data, and distributes time between nodes with no data evenly between \DIFdelbegin \DIFdel{nodes with calibration data.
This minimizes }\DIFdelend \DIFaddbegin \DIFadd{calibrated nodes.
This has proven effective to minimize }\DIFaddend age variance in the resulting chronogram \DIFaddbegin \DIFadd{and useful for ecological analyses }\DIFaddend (Webb et al., 2008).
...
BLADJ requires a root age estimate. If there is no information on the age of the root in the chronogram database, users can provide an estimate from the literature. If none is provided, DateLife \DIFdelbegin \DIFdel{assigns an arbitrary age to the root as 10\% older than the oldest age available within the group}\DIFdelend \DIFaddbegin \DIFadd{will not return a dated topology but provide a warning message along with suggestions on how the user can provide an age for the root so DateLife can run.
}

\DIFadd{Alternatively, the user can use the MrBayes (Huelsenbeck \& Ronquist, 2001; Ronquist \& Huelsenbeck, 2003) approach in DateLife uses the calibrations as priors on node ages}\DIFaddend .
