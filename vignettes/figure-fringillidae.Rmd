---
title: "Graphs for figure of all fringillidae v2022.01.28"
...
```
```{r, eval = FALSE}
dquery_ott <- datelife::make_datelife_query(input = "fringillidae",
                                             use_tnrs = TRUE,
                                             get_spp_from_taxon = TRUE,
...
```{r, eval = TRUE}
dsumm <- fringillidae_v2022.01.28$dsumm
```
...
# TODO: trace back from where this calibration_distribution comes from

head(congruified_calibs)
# usethis::use_data(congruified_calibs, overwrite = TRUE)
# a data frame of 818 rows and 8 columns
```
...
# usethis::use_data(phylo_median_congruified)
...
head(calibration_distribution_congruified)
length(calibration_distribution_congruified)
```

TODO: verify that `check_conflicting_calibrations` is working good and being used where needed

```{r, eval = FALSE}
test1 <- datelife:::check_conflicting_calibrations(phy = phylo_median_congruified,
...
```
Saving the congruified table as data

```{r, eval = FALSE}
congruified_calibrations_v2022.01.28 <- pretty_calibrations

usethis::use_data(congruified_calibrations_v2022.01.28, 
                  overwrite = TRUE)
```
...
```{r}
summary_ages <- datelife::summarize_congruifiedCalibrations(my_calibrations)
head(summary_ages)
nrow(summary_ages)
```
...
sum(has_age_data)
summary_ages <- read.csv("../tables/table-fringillidae-all-summary.csv")
colnames() <- gsub("[.]", " ", colnames(summary_ages))
head(summary_ages)
nrow(summary_ages)
data.frame(node_name = summary_ages$`Node Name`,
           median = summary_ages$`Median Age`, 
           times = ape::branching.times(phylo_median_congruified)[has_age_data])

chronogram_ages <- round(ape::branching.times(phylo_median_congruified)[has_age_data], digits = 4)
length(chronogram_ages) # 194
median_equals_branching_time <- round(summary_ages$`Median Age`, digits = 4) == chronogram_ages

node_ages_used <- summary_ages$`Node Name`[median_equals_branching_time]
length(node_ages_used) # 173 ages not used as calibration

node_ages_unused <- summary_ages$`Node Name`[!median_equals_branching_time]
length(node_ages_unused) # 21 ages not used as calibration
node_ages_unused
```
...
# usethis::use_data(fringillidae_all_ages)
