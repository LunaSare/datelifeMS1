...
<!-- 
Writing congruified calibrations as PDF??

```{r}
## set the theme to have italics in species names:
number_of_rows <- nrow(pretty_calibrations)
tt2 <- gridExtra::ttheme_default(core=list(fg_params=list(fontface=c(rep("plain", number_of_rows), rep("italic", number_of_rows*2), rep("plain", number_of_rows*2)))))
```
-->

Table of summary statistics of congruified ages:

```{r}
colnames(pretty_calibrations)
only_calibs <- pretty_calibrations[,c("Node Name", "Node Age")]
colnames(only_calibs) <- c("node", "age")
class(only_calibs$`node`)
summary(only_calibs)
```

Creating summary table with a function:
...
colnames(summ_calibs$in_phy) <- c("mrca_node_name",
                            ...
                            "nodeAge",
                            "MaxAge")
summ_calibs$in_phy$reference <- rep("Median of node ages",
                                    nrow(summary_ages))
...
names(color_median) <- summ_calibs$in_phy$reference[1]
```
...
all_ages <- list(a1 = cc_calibs, a2 = summ_calibs)
...
Plot:
TODO: 
- show only congruified ages - DONE
- use asterisks to show median ages that were not used as calibrations.
- use different symbols for different studies

```{r}
#  devtools::load_all("../../datelifeplot")
svg(filename = paste0(writing_dir, "/median_and_calibration_ages-congruified.svg"),
    height = 30,   
    width = 15)
