...
These studies correspond to indices 1, 4 and 5 from the datelife result, so we are going to pluck those out:
...
```
The following gives an aerror, check:
```{r eval = FALSE}
...
If we add a target tree, it works fine:
```{r}
...
all_calibrations
```
`all_calibrations` is the table of nodes from the source chronograms, that is why it is missing actual node names from the target tree:

```{r}
colnames(all_calibrations)
```

Congruified node ages are in calib_summ$in_phy:
```{r}
...
Extract columns from `calib_summ$in_phy` to show in figure 1
```{r}
pretty_table <- data.frame(node_name = 1, calib_summ$in_phy[,c("nodeAge", "reference")])
pretty_table$node_name <- paste0("n", calib_summ$in_phy$mrca_node_number - ape::Ntip(phylo_median))
pretty_table
rr <- order(pretty_table$node_name, pretty_table$reference)
pretty_table <- pretty_table[rr,]
colnames(pretty_table) <- c("Node Name", "Node Age", "Study chronogram")
rownames(pretty_table) <- 1:11
```
...
tt1 <- gridExtra::ttheme_default(core=list(fg_params=list(fontface=c(rep("plain", 33)))))

# Export PDF
pdf("../figures/figure-workflow/congruified_nodes_table.pdf", 
    width = 5, height = 4)
gridExtra::grid.table(pretty_table, theme = tt1) # if you do not want row numbers set rows = NULL
dev.off()
```
...
I found some errors in the table on Fig 1, so there must be a bug in the way I was calculating the median summary tables above.
I am using the DateLife approach next:
...
for (i in unique(calib_summ$in_phy$mrca_node_name)) {
  rr <- i == calib_summ$in_phy$mrca_node_name
  xx <- summary(calib_summ$in_phy$nodeAge[rr])
  res <- c(res, list(xx))
  summ_sd <- c(summ_sd, sd(calib_summ$in_phy$nodeAge[rr]))
}

# go from list to data frame by row:
summ_table <- as.data.frame(do.call(rbind, res)) 

summ_table$node_name <- paste0("n",
                                      unique(calib_summ$in_phy$mrca_node_number) - ape::Ntip(phylo_median))

summ_table$sd <- summ_sd
colnames(summ_table)
pretty_summ_table <- summ_table[, c("node_name", "Min.", "Median", "Mean", "Max.", "sd")]
colnames(pretty_summ_table) <- c("Node", "Min. Age", "Median", "Mean", "Max. Age", "SD")

pdf("../figures/figure-workflow/summary_table_updated.pdf", height = 2)       
gridExtra::grid.table(pretty_summ_table, rows = NULL) # if you do not want row numbers set rows = NULL
dev.off()
```
...
```
Something odd with median ages and the tree: 
```{r}
data.frame(node = mm_summ$in_phy$mrca_node_name,
           age = mm_summ$in_phy$nodeAge,
           median = summ_table$Median, 
           median_node = summ_table$node_name,
           median_phylo = ape::branching.times(small_phylo_median)[summ_table$node_name])
```

Remake the tree with correct median on node 1:

```{r}
topology <- small_phylo_median
topology$edge.length <- NULL
topology
plot(topology)
ape::nodelabels(text = topology$node.label)
small_phylo_median_new <- datelife::make_bladj_tree(tree = topology, 
                          nodenames = summ_table$node_name, 
                          nodeages = summ_table$Median)
plot(small_phylo_median_new)
ape::nodelabels(text = topology$node.label)
ape::branching.times(small_phylo_median_new)
```

Fix `mm_summ`
```{r}
mm_summ$in_phy$nodeAge[1] <- summ_table$Median[1]
```
...
#  source("../R/plot_node_ages_v2.3.R")
```
...
svg(filename = "../figures/figure-workflow/all_ages.svg",
    height = 4,
    width = 6)
plot_node_ages_2.3(chronogram = small_phylo_median_new,
                   title = c("Median Summary Chronogram"),
