...
but it seems to be dropped on the extract_calibrations step:
...
```
The mismatch comes from phylogenetic conflict in the median tree and the Hedges tree. 
Even though the median tree uses the node ages for "Carpodacus_roseus"/"Loxioides bailleui" pair, when the median matrix is clustered, the new relationships conflict with the clade that includes "Carpodacus_roseus"/"Loxioides bailleui" in Hedges.
...
Next steps: plot the actual input data on top of the median tree, and SDM tree.
