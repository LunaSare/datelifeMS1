...
sd_ages <- c()
var_ages <- c()
for (node in unique(summary_table$`Node Name`)) {
...
summary_table$sd <- sd_ages
summary_table$Variance <- var_ages
```

Tried the following but is too complicated and does not work well, so tried something else.
```
# Create an array of summary statistics per node
summary_array <- tapply(only_calibs$`Node Age`, only_calibs$`Node Name`, summary)
class(summary_array)
class(summary_array[1])

# Convert to list
summary_list <- lapply(summary_array, "[")
class(summary_list)
dim(summary_list)
length(summary_list)
class(summary_list[[1]])

as.data.frame.table(summary_list[[1]])
# Elements of list are class table, not data.frame
# Merge with rbind.data.table
xx <- data.table:::rbind.data.table(summary_list)
class(xx)
ls(xx)
class(xx$n1)
```

2. Update tables of figure of all Fringillidae, add to Supplementary data
3. update figure 5, especially points of source ages, identify which ones were used and which ones were excluded.
4. Choose one figure to showcase results of cross validation analyses
5. Add other cross validation figures to supplements
6. Check that cross validation is run without chronograms from the same study, especially for study from Price etal. 2014, chronogram 2
7. Address remaining comments
8. Submit!!!
