...
if (any(duplicated(taxon_pairs_ref))) {
  pretty_calibs <- pretty_median[!duplicated(taxon_pairs_ref),]
}
...
tt1 <- gridExtra::ttheme_default(core=list(fg_params=list(fontface=c(rep("plain", 28), rep("italic", 28*2), rep("plain", 28*2)))))

# Export PDF
pdf("../figures/figure2/calibs_table.pdf", height = 10, width = 10)
...
calib_summ <- datelife:::summary.matchedCalibrations(matched_median_ages$matched_calibrations)
...
```{r}
colnames(calib_summ$in_phy)
pretty_median <- calib_summ$in_phy[,c("taxonA", "taxonB", "nodeAge", "mrca_node_number")]
pretty_median
# Eliminate duplicated taxon pairs:
...
# Modify column names to be informative
colnames(pretty_median) <- c("Node Name", "taxon A", "taxon B", "Median Age")
# Change node numbers my node names
...
# Export PDF
pdf("../figures/figure2/median_table.pdf")
...
gridExtra::grid.arrange(layout_matrix = pretty_median, theme = tt1) 

# Getting the median per node:
median_singletons <- c()
for (i in unique(pretty_median$`Node Name`)) {
  print(i)
  x <- median(pretty_median$`Median Age`[pretty_median$`Node Name` == i])
...
pretty_median$`Median Age`[duplos] <- ""
pretty_median$`Median Age`[!duplos] <- median_singletons
pretty_median$`Median Age`[1] <- ""
pretty_median$`Median Age`[5] <- median_singletons[1]

# Writing the table out:
pdf("../figures/figure2/median_table_simple.pdf")       # Export PDF
gridExtra::grid.table(pretty_median, theme = tt1) # if you do not want row numbers set rows = NULL
dev.off()
```
...
Plot again:
```{r}
svg(filename = "../figures/figure2/median_and_calibration_ages_simple.svg",
    height = 8.5 ,
    width = 10.5)
datelifeplot::plot_node_ages2(phylo_median,
               title = c("Median Summary Chronogram"),
